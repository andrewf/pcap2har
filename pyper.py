...
#delete
class FlowStateTransformer(object):
    def __init__(self, flow_states, regions):
        flow_states.sort()

        #flow_states.sort(lambda x,y: y[1]-x[1] if x[0]==y[0] else x[0]-y[0])

        disjoint_states = []
        idx = 0

        # goal: build disjoint_states out of segments that share the same set of
        # overlapping states.
        # preconditions: 
        # - for all flow_states i and j with i < j:
        #      flow_states[i][0] < flow_states[j][0] or
        #      (flow_states[i][0] == flow_states[j][0] and flow_states[i][1] <= flow_states[j][1])
        # - for all flow_states i, flow_states[i][0] <= flow_states[i][1]
        # invariants:
        # - ts >= flow_states[idx][0] and ts <= flow_states[idx][1]
        # - for all disjoint_states st with st[0] <= ts, st[2] is the intersection of the
        #   flow_states up to idx, up to ts
        # - disjoint_states[-1][1] == ts (if disjoint_states[-1] exists yet)
        # step function:
        # - each iteration adds an entry to disjoint_states with entry[0]==ts and 
        #   entry[2][0]==flow_states[idx][2] and advances either ts or idx (or both)
        ts = flow_states[0][0]
        last_ts = ts - 0.001
        while idx < len(flow_states):
            idx_start, idx_stop, idx_state = flow_states[idx]
            states = [idx_state]
            min_tail = idx_stop
            j = idx+1
            while j < len(flow_states):
                jstart, jstop, jstate = flow_states[j]
                if min_tail <= jstart:
                    break
                if ts >= jstart and ts < jstop:
                    states.append(jstate)
                    if jstop < min_tail:
                        min_tail = jstop
                elif ts <= jstart and last_ts != ts:
                    min_tail = jstart
                    break
                #log.critical("%d/%d: %f-%f, %s, (%f->%f, %s)" % (idx, j, jstart, jstop, jstate, ts, min_tail, `[s.color for s in states]`))
                j += 1
            #log.critical("done %d/%d(%d): %f->%f, %s" % (idx, j, len(flow_states), ts, min_tail, `[s.color for s in states]`))
            disjoint_states.append((ts, min_tail, states))
            while idx < j and flow_states[idx][1] <= min_tail:
                idx += 1
            last_ts = ts
            ts = min_tail
        ...
        self.flow_colors = []
        prev_start = 0.0
        prev_stop = disjoint_states[0][0]
        idx = 0
        for start, stop, states in disjoint_states:
            self.flow_durations.append(stop-start)
            self.flow_colors.append(states[-1].color)
            #if prev_start > stop or prev_stop != start or start > stop:
            #    log.debug("BAD ENTRY: %f-%f, then %f-%f (idx=%d)"%(prev_start, prev_stop, start, stop, idx))
            prev_start, prev_stop = start, stop
            idx += 1
        
        self.region_view_list = []
        idx = 0
        for reg_start, reg_stop in regions:
            pair_durations = []
            pair_colors = []
            while idx < len(disjoint_states):
                idx_start, idx_stop, idx_states = disjoint_states[idx]
                if reg_start < idx_stop:
                    break
                idx += 1
            for i in range(idx, len(disjoint_states)):
                istart, istop, istates = disjoint_states[i]
                if istart >= reg_start and istart < reg_stop:
                    pair_durations.append(min(istop, reg_stop)-istart)
                    pair_colors.append(istates[-1].color)
                elif istart >= reg_stop:
                    break
            self.region_view_list.append((pair_durations, pair_colors))

    def full_view(self):
        return self.flow_durations, self.flow_colors

    def region_views(self):
        for pair_durations, pair_colors in self.region_view_list:
            yield pair_durations, pair_colors


#delete
def append_pair_states(flow_states, req_start, req_end, resp_start, resp_end, rtt_oneway):
    """
    Function to setup the flow states for a single request/response pair.

    rtt_oneway should be 
    """

    # Reject impossible combinations of start and end times
    # should raise here, i think. i'm too lazy but wanted useful data in the msgs.
    if req_start > req_end:
        log.critical('BAD REQUEST %f-%f' % (req_start, req_end))
    if req_end > resp_start:
        log.critical('BAD REQUEST-RESPONSE GAP %f-%f' % (req_end, resp_start))
    if resp_start > resp_end:
        log.critical('BAD RESPONSE %f-%f' % (resp_start, resp_end))

    # Take care of the request (it's simple)
    flow_states.append((req_start, req_end, fs_request))

    # Take care of the wait between request and response
    wait_start = req_end
    wait_end = resp_start
    # If we don't have rtt info, or it's bogus, just use the timestamps we have
    if rtt_oneway is None or rtt_oneway <= 0.0:
        flow_states.append((wait_start, wait_end, fs_wait))

    # If we actually have good info, try to calculate the network transit time
    else:
        if rtt_oneway*2 >= wait_end-wait_start:
            # throw here?  we're inconsistent
            flow_states.append((wait_start, wait_end, fs_net_delay))
        else:
            first_rtt_end = wait_start + rtt_oneway
            backend_end = wait_end - rtt_oneway
            flow_states.append((wait_start, first_rtt_end, fs_net_delay))
            flow_states.append((first_rtt_end, backend_end, fs_wait))
            flow_states.append((backend_end, wait_end, fs_net_delay))
    #flow_states.append((req_end, resp_start, fs_wait))

    # Actual response
    flow_states.append((resp_start, resp_end, fs_response))

fs_unstarted = FlowState('', '#aaaaaa')
fs_dnsquery = FlowState('dns query', '#ffff00')
fs_handshake = FlowState('tcp handshake', '#EC7612')
fs_handshake_done = FlowState('', '#3765D9')
fs_request = FlowState('request', '#9E42EE')
fs_net_delay = FlowState('net delay', '#cc5555')
fs_wait = FlowState('wait for server', '#cc0000')
fs_response = FlowState('response', '#00cc00')
fs_idle = FlowState('connection idle', '#3765D9')
fs_stalled = FlowState('connection stalled', '#002200')
fs_paused = FlowState('connection paused', '#008800')
fs_finished = FlowState('', '#eeeeee')
...
class HTTPFlow(TCPFlow):
    ...
    def build_states(self):
        ...
        st = FlowStateTransformer(self.flow_states, regions)
